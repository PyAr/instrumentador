...
def merge_dicts(frame):
    a = {}
    a.update(deepcopy(frame.f_builtins))
    a.update(deepcopy(frame.f_globals))
    a.update(deepcopy(frame.f_locals))
    return a

def print_diff(last, new):
    print(DeepDiff(last, new, ignore_order=True))

def get_value(name, frame):
...
class Trace:
    ...
    def globaltrace_lt(self, frame, why, arg):
        ...
        """
        #print("*** :( ooo" + why, frame, arg)
        if why == 'call':

            call_stack.append(merge_dicts(frame))
    ...
    def localtrace_trace(self, frame, why, arg):
        ...
        if why == "return":
            last_frame = call_stack.pop()

        if why == "line":

            if call_stack:
                last_frame = call_stack.pop()
                print_diff(last_frame, merge_dicts(frame))
            ...
            print("%s(%d): %s" % (bname, lineno,
                                  line.strip()), end='')
            try:
                t = ast.parse(textwrap.dedent(line))
            except Exception as err:
                t = ast.parse(textwrap.dedent(line) + " pass")
            finally:
                names = [n.id for n in ast.walk(t) if isinstance(n,  ast.Name)]
                values = [get_value(name, frame) for name in names]
                print("  --   " + ", ".join("%s=%s" % (n,v) for n,v in zip(names, values)))
            print()
        return self.localtrace
